...
//				System.out.println(s);
				...
				Set<Integer> rightRolesList = roleRightService.findRolesByRightCode(code);
				authorized = checkHasPermission(userRoles, rightRolesList);
			}
			else
			{
	...
	private boolean checkHasPermission(List<SimpleGrantedAuthority> userRoles, Set<Integer> rightRolesList)
	{
		boolean flag = false;
		...
		if(null != rightRolesList && rightRolesList.size() > 0)
		{
			for(SimpleGrantedAuthority userRole : userRoles)
			{
				if(rightRolesList.contains(userRole.getAuthority()))
				{
					flag = true;
					break;
				}
